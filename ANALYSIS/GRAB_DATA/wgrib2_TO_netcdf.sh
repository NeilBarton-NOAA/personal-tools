...
#VARS="TMP UGRD VGRD DSWRF DLWRF USWRF ULWRF ICEC"
VAR='ICEC'
taus=( $(seq 0 6 840) )
#taus=( $(seq 0 24 48) )
...
for EXP in "${!EXPS[@]}"; do
    ...
    while [[ $last_tau != ${taus[@]: -1} ]]; do
        ...
        #for tau in ${ptaus[@]}; do
        #    tau=$(printf "%03d" $tau)
        #    files=$(ls ${EXPS[$EXP]}/*/gfs*/00/atmos/*${tau}.grib2 )
        #    nc_file=${SAVE_DIR}/${VAR}_${tau}.nc
        #    temp_nc_file=${SAVE_DIR}/temp_${VAR}_${tau}.nc
        #    if [[ ! -f ${nc_file} ]]; then
